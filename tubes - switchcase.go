	...
	for i < N {
		for j < M {
			if datakursiV[i][j].kodereservasi == kodereservasi {
				fmt.Println("\nData kode reservasi yang anda cari:")
	...
	for i < N {
		for j < M {
			if datakursiV[i][j].noiden == noiden {
				fmt.Println("\nData nomor identitas yang anda cari:")
				outputdatapenontonVIP()
			}
			j = j + 1
		}
...
func menampilkan_datapenontonVIP_berdasarkan_tempatduduk(){
	...
	if datakursiV[i][j].kodereservasi != 0 {
		fmt.Println("\nInformasi kursi yang anda cari:")
		outputdatapenontonVIP()
	} else {
...
func main() {
	...
	for nomor >= 1 && nomor <= 8 {
		...
		case nomor == 4:
			menampilkan_datapenontonVIP_berdasarkan_tempatduduk()
		case nomor == 5:
